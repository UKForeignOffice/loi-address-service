...
const httpInternalErrorStatusCode = 500
...
router.route('/lookup/:postcode').get(async (req, res) => {
  ...
  try {
    ...
    const response = await axios.get(`${authConfig.url}/Find/v1.10/json3.ws`, {
      params,
    })
    ...
    if (postcodeLookups.length > 0) {
      ...
      const postcodeRequests = postcodeLookups.map((Id) => {
        ...
        return axios.get(`${authConfig.url}/Find/v1.10/json3.ws`, {
...
function hasAddressItems(response) {
